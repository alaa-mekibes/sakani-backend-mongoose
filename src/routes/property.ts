...
import { uploadProperty } from "../middleware/upload";
...
router.post("/", authMiddleware, uploadProperty.array('images', 5), validate(createPropertySchema), (req, res) => propertyController.addProperty(req, res));
router.patch("/:propertyId", authMiddleware, uploadProperty.array('images', 5), validate(updatePropertySchema), validate(propertyIdSchema, 'params'), (req, res) => propertyController.updateProperty(req, res));
